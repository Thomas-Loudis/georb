function [orbc,err,veqZarray,veqParray] = gauss_jackson_main(zo,arc,MSparam,RKparam,eopdat,dpint)


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Integration of Equation of Motion and Variational Equations based on
% Gauss-Jackson integrator method
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Input arguments
...
%  gj = [g1...gj...gm]
% Variable matrix g starts with index 1.
%  Examble: go = g(1,1)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
[orbcRK,errRK,veqZarrayRK,veqParrayRK] = intgveq_rk(zo,arcRK,RKparam,eopdat,dpint);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Accleration function steps
    accl(istart,:) = [fx fy fz];
    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% VEQ Function evaluations for the first steps 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Matrix (3D) of 2nd Derivatives of the State Transition Matrix (veqZr_dv2)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Matrices Preallocation
orbc = zeros(arc/h+1 , 7);
arc_ms = arc - arcRK;
orbc_ms = zeros(arc_ms/h , 7);
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Orbit Integration based on Gauss-Jackson method
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Integration : VEQ
    ...
    [V_veqZr_dv2] = intgveq_msgj2(veqZo_ms,veqZr_dv2,gj_ast,dj_ast,MSparam);
    [veqZ] = intgveq_msgj(V_veqZr_dv2,gj,dj,MSparam);
    %clear V_veqZr_dv2
    
    % Sensitivity Matrix
    [V_veqPr_dv2] = intgveq_msgj2(veqPo_ms,veqPr_dv2,gj_ast,dj_ast,MSparam);
    [veqP] = intgveq_msgj(V_veqPr_dv2,gj,dj,MSparam);    
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% Accleration Function evaluations steps
    [na n2] = size(accl);
    accel_interm = zeros(na,n2);
    %accel_interm(1:na-1,:) = accl(2:na,:);
    for i1 = 1 : na-1
        for i2 = 1 : n2
...
% Corrector : VEQ Function evaluations for first steps
    % State Transition matrix' 2nd Derivatives
    veqZr = veqZ(1:3,:);
    ...
    % 2nd Derivatives of Sensitivity Matrix  (veqPr_dv2)
    veqPr = veqP(1:3,:);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Orbit & State vector for next epoch TT
...
% VEQ Function evaluations for next point's first steps    
    % 2nd Derivative of State Transition matrix  (veqZr_dv2)
    veqZr = veqZ(1:3,:);
    veqZr_dv2_next_epoch = pdv_acc * veqZr;
    [sz1, sz2, sz3] = size(veqZr_dv2);    
    veqZr_dv2_interm = zeros(sz1, sz2, sz3);        
